DROP VIEW IF EXISTS vw_inference_input_daily;
CREATE VIEW vw_inference_input_daily AS
SELECT
  event_id,
  fecha_evento,
  proveedor_candidato,
  ...
  mes,
  fin_mes
FROM public_dataset_modelo_proveedor_v2_candidates;

DROP VIEW IF EXISTS vw_event_summary_daily;
CREATE VIEW vw_event_summary_daily AS
SELECT
  event_id,
