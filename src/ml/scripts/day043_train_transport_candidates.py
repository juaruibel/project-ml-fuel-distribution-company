...
VARIANT_NAME_MAP = {
    day043_builder.TRANSPORT_CARRY_VARIANT: "V2_TRANSPORT_CARRY30D_ONLY_LR_smote_0.5_v1",
    day043_builder.DISPERSION_PLUS_TRANSPORT_VARIANT: "V2_DISPERSION_PLUS_TRANSPORT_CARRY30D_LR_smote_0.5_v1",
}


# SECTION: CLI
def parse_args() -> argparse.Namespace:
    """Parse CLI arguments for the Day 04.3 training pipeline."""
    parser = argparse.ArgumentParser(
        description="Entrena variantes Day 04.3 solo si el gate de cobertura/leakage lo permite."
    )
    parser.add_argument(
        "--quality-report",
        type=Path,
        default=Path("artifacts/public/data_quality_day043_transport_matrix.json"),
        help="Reporte de calidad Day 04.3 generado por el builder.",
    )
    parser.add_argument(
        "--registry-csv",
        type=Path,
    ...
    parser.add_argument(
        "--candidate-model-dir",
        type=Path,
        default=Path("models/candidates/day043_transport"),
        help="Directorio raiz para artefactos Day 04.3.",
    )
    parser.add_argument(
...
def _summary_output_path(reports_root: Path, run_id: str) -> Path:
    """Build the persisted Day 04.3 run summary path consumed by notebook 14."""
    return reports_root / "metrics" / "candidates" / run_id[:8] / f"{run_id}_day043_transport_run_summary.json"
...
def _variant_features_from_quality(
    ...
    """Resolve numeric/categorical/target columns for one Day 04.3 variant."""
...
def _baseline_metrics_from_registry(registry_csv: Path) -> dict[str, float]:
    """Load baseline metrics to report Day 04.3 deltas in the run summary."""
...
def run_pipeline(args: argparse.Namespace) -> dict[str, Any]:
    """Train Day 04.3 candidates only when the dataset gate allows it."""
    project_root = Path(__file__).resolve().parents[3]
    ...
    if not gate_payload.get("training_allowed", False):
        ...
        summary["message"] = "Day 04.3 no entrena porque el artefacto carry-forward 30d no supera el gate tecnico."
        write_json(_summary_output_path(reports_root=reports_root, run_id=run_id), summary)
        return summary

    trained_results: list[dict[str, Any]] = []
    for variant_name in [day043_builder.TRANSPORT_CARRY_VARIANT, day043_builder.DISPERSION_PLUS_TRANSPORT_VARIANT]:
        variant_info = quality_payload["variants"][variant_name]
    ...
    if not trained_results:
        summary["status"] = "gated_no_train"
        summary["message"] = "Day 04.3 no encontro variantes construidas para entrenar."
        write_json(_summary_output_path(reports_root=reports_root, run_id=run_id), summary)
        return summary
    ...
    summary.update(
        {
            ...
            "transport_only_dataset": quality_payload["variants"][day043_builder.TRANSPORT_CARRY_VARIANT]["dataset_path"],
            "transport_plus_dispersion_dataset": quality_payload["variants"][
                day043_builder.DISPERSION_PLUS_TRANSPORT_VARIANT
            ]["dataset_path"],
        }
    )
    write_json(_summary_output_path(reports_root=reports_root, run_id=run_id), summary)
...
def main() -> None:
    """Run the CLI entrypoint for the Day 04.3 training pipeline."""
